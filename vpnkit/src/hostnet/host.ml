  ...
  module Stream = struct
    ...
    module Tcp = struct
      ...
      let connect ?read_buffer_size:_ (ip, port) =
        ...
        | Error (`Msg m) ->
            let msg =
              Fmt.str "Socket.%s.connect %s:%d: %s" label (Ipaddr.to_string ip)
                port m
            in
            Log.info (fun f -> f "%s" msg);
            Lwt.return (Error (`Msg msg))
      ...
      let bind_one ?(description = "") (ip, port) =
        ...
        | Error (`Msg m) ->
            let msg =
              Fmt.str "Socket.%s.bind_one %s:%d: %s" label (Ipaddr.to_string ip)
                port m
            in
            Log.err (fun f -> f "%s" msg);
...
module Time = struct
  let sleep_ns ns =
    Luv_lwt.in_luv (fun return ->
        ...
        | Ok timer -> (
            match
              Luv.Timer.start timer (Duration.to_ms ns) (fun () ->
                  return (Ok ()))
            with
            | Error err -> return (Error (`Msg (Luv.Error.strerror err)))
            | Ok () -> ()))
                    ...
                    | `INET -> (
                        ...
                        | Some ip -> (
                            ...
                            | Error (`Msg m) ->
                                Log.err (fun f ->
                                    f
                                      "getaddrinfo %s returned invalid IP %s: \
                                       %s"
                                      node ip m);
                                acc
                            | Ok ip -> ip :: acc))
...
let start_background_gc config =
  ...
  | Some s -> (
      ...
      | Error err ->
          Log.err (fun f ->
              f "Unable to configure periodic Gc.compact: %s"
                (Luv.Error.strerror err))
      | Ok timer -> (
          match Luv.Timer.start timer (5 * 1000) ~repeat:(s * 1000) compact with
          | Error err ->
              Log.err (fun f ->
                  f "Unable to start periodic Gc.compact: %s"
                    (Luv.Error.strerror err))
          | Ok () -> ()))
