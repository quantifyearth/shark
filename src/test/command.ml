...
let test_generic_command_basic () =
  ...
  let expected =
    Command.v ~name:"docker" ~args:[ "docker"; "arg1"; "arg2" ] ~file_args:[]
  in
  let test = Command.of_string testcase in
  Alcotest.(check (option command)) "Command" (Some expected) test;
  Alcotest.(check string)
