...
let command = Alcotest.of_pp Ast.pp
...
```
  |}
  in
  let test, _ = Shark.Md_to_ast.of_sharkdown template_markdown in
  let astblocks = Ast.to_list test in
  Alcotest.(check int) "Single command group expected" 1 (List.length astblocks);
  let leaves = Ast.Block.commands (List.nth astblocks 0) in
